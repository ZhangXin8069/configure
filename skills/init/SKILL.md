...
name: init
description: |
  当用户要求初始化仓库或目录、生成或更新 AGENTS.md、归档 agent 配置、收集 skill 或整理目录，
  或说“init”“初始化”时使用。
metadata:
  openclaw:
    emoji: 📦
...
| 重命名目标已存在 | 以当前秒级时间戳重试 |
| 执行中途中断 | 重新运行即可：阶段 A 只读无副作用，阶段 B 幂等（已归档/已归集的自动跳过）；中断与失败原因在终端摘要中说明 |
| 阶段 B 多次失败 | 基于终端摘要与命令输出定位失败步骤（重命名冲突、权限、磁盘等），修正后重新执行（循环尝试直至成功）；无法解决时向用户报告 |
...
- 本次改动按公共 Git 契约检查，未经明确要求不自动提交或推送；涉及 git 跟踪文件的重命名与删除仍须遵守用户授权；
