...
5. **不确定就问**：无法判定外来文件夹/agent 生成内容时询问用户，不臆断。
...
```bash
ls -1t .init.*.log 2>/dev/null | head -3                 # 按时间倒序列出最近日志(优先较新, 最多3份)
tail -80 "$(ls -1t .init.*.log 2>/dev/null | head -1)"   # 预读最新一份的尾部（汇总区）
```

- 默认预读**最新一份**日志的尾部（结构化汇总、结论、遗留项）；需更多对照时按时间倒序逐份追加读取，不一次全读
- 目的：① **快速上手**——从上次汇总直接掌握已初始化/已归档/已归集状态，幂等判定与遗留项直接沿用，
  避免重复调查；② **对照参考**——上次的失败与重试原因、清单外新登记项供本次对照，防止重复踩坑
...
4. **识别生成此内容的 skill**（针对 Step 2.3 确认的内容）：
   ...
   - 若无法识别但确为 agent 生成：询问用户；或根据内容反向撰写一份完整 skill
     （说明该内容是如何生成的）。
5. **扫描散落 skill**（全库只读）：
...
| 无法判定内容是否 agent 生成 | 询问用户，不臆断 |
| 无法定位生成内容的 skill | 询问用户；或据内容撰写完整 skill |
...
- 不确定（外来文件夹判定、是否 agent 生成、生成技能的归属）时先询问用户，不臆断；
