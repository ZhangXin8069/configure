...
9. **只读分析**：分析阶段不改动仓库任何内容；唯一写操作是 `docs/` 下的
   `.tex` 源文件与编译产物 `.pdf`。
10. **图表与日志穷尽性（大任务产出详览）**：报告必须完整收录本次大任务中产生的全部图表与全部日志关键内容，每项配详尽介绍（来源与生成方式、数据含义、逐项解读、与核心结论关联、局限），置于合理位置——结果分析中逐图逐日志解读、综合分析中关联、附录中原始清单与原始片段——不得遗漏；以清单闭合为交付前提。
...
1. **一条主线**：摘要（点题+性质判定）→ 项目定位与核心部分清单（穷尽枚举）→
   核心部分深度剖析（逐部分）→ 独特性评估 → 关键片段与公式 → 本次大任务图表与日志详览（穷尽清单+逐项详介，置于结果/综合分析与附录合理位置） → 参考源清单 → 结论；
...
8. **图表与日志穷尽详览（大任务必备）**：本次大任务全部图表与全部日志关键内容逐项落位——结果分析中逐图逐日志解读、综合分析中关联、附录中原始清单与 `\lstinputlisting` 片段；每项含来源、生成方式、数据含义、详尽解读与结论关联，配 `文件:行号` 或 `日志:行号` 参考源；以清单闭合为交付前提。
