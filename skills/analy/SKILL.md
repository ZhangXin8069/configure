...
description: |
  ...
  （物理图像/业务实体），或想要一份可追溯（文件:行号）、带统计与结论的正式分析文档，即使
  未明确说出"分析"字样（如"这个仓库是干嘛的"、"帮我梳理 env.sh 加载链"、"讲下这段格点代码
...
2. **解析用户输入**：把 `{$用户输入}` 解析为分析主题——提取问题焦点、限定范围与期望输出；
   输入为明确主题时直接执行；输入含糊时**先提问**，不臆断主题。
3. **先全库调查，后主题聚焦**：先建立仓库全景（目录结构、git 历史、文档与代码索引），
   再按主题收敛到相关文件——全景索引是防止遗漏关联证据的保险，先广后深。
...
生成 `docs/analy_<slug>_<YYYYMMDD>.tex`，严格遵循下方「报告模板与生成要点」的模板结构：
一条主线（摘要→概览→主题分析→关键片段→参考清单→结论）、层次分明、五色统一标识、
`\lstinputlisting` 代码保真、`\detokenize` 路径防转义。
...
报告 `.tex` 遵循以下模板结构（中文内容用 xelatex + ctex；纯英文可退化为 article + xelatex）：
...
- [ ] 代码片段全部用 `\lstinputlisting` 直引（无手抄转写）
- [ ] 多资料主题含"联系与层次"呈现（层级表/依赖链）；代码主题含"对象映射"表
- [ ] 报告结构遵循模板主线，五色标识统一，无额外颜色
- [ ] 正文特殊字符已转义，路径走 `\detokenize`/`\lstinputlisting`
- [ ] PDF 编译成功且产物存在非空；`pdftotext` 抽检标题/结论/参考源出现
- [ ] 总结输出覆盖：主题/覆盖/参考/证据/产物/结论/局限
- [ ] 无依据结论已如实标注"未找到相关依据"，未编造任何证据
