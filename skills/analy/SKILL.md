...
10. **图表与日志穷尽性（大任务产出详览）**：报告必须完整收录本次大任务中产生的全部图表与全部日志关键内容，每项配详尽介绍（来源与生成方式、数据含义、逐项解读、与结论关联、局限），置于合理位置——结果分析中逐图逐日志解读、综合分析中关联、附录中原始清单与原始片段——不得遗漏；以清单闭合（图表清单、日志清单）为交付前提，缺一即未完成。
...
12. **图表与日志穷尽详览（大任务必备）**：每次大任务产生的全部图表与全部日志关键内容必须逐项落位——结果分析（A9/B9）中逐图逐日志解读、综合分析中关联、附录中原始清单与 `\lstinputlisting` 片段；每项含来源、生成方式、数据含义、详尽解读与结论关联，配 `文件:行号` 或 `日志:行号` 参考源；以“图表清单闭合、日志清单闭合”为交付前提，缺一即返工。
...
本次大任务图表与日志详览（穷尽清单+逐项详介，置于结果/综合分析与附录合理位置）→
参考源清单 → 结论与局限（收束）。
...
- [ ] 本次大任务全部图表与全部日志关键内容已穷尽收录（图表清单、日志清单闭合），每项配详尽介绍（来源/生成方式/含义/解读/关联）并置于合理位置（结果分析逐项解读、综合分析关联、附录原始清单）
