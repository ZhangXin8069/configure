...
7. **编译闭环与防溢出**：生成 `.tex` → 编译 → 验证 PDF 产物存在且内容正确；
   交付前 `grep -c Overfull` 计数必须为 **0**（长代码/长表/长词/公式均按模板防溢出规则处理，
...
8. **防溢出**：`grep -c Overfull` 交付前必须为 0——长表用 longtable/X 列、
   长词靠 `\emergencystretch`+microtype、彩色框全部 breakable、公式用 align 分行；
...
**各部分关系**（织网）→ **项目思路**（灵魂）→ 主题分析（深入）→ 关键代码/文档片段（证据）→
参考源清单 → 结论与局限（收束）。
...
**防溢出底线**：`grep -c Overfull` 交付前必须为 0；所有措施（长代码 ≤40 行/段、
longtable/X 列长表、`\emergencystretch`+microtype、breakable 框、align 分行公式）
见模板第 4 节，不自行删减。
...
- [ ] PDF 编译成功且产物存在非空；`grep -c Overfull` = 0；`pdftotext` 抽检标题/结论/参考源出现
...
- 交付前 `grep -c Overfull` 必须为 0（模板防溢出规则见 references/latex-template.md）；
