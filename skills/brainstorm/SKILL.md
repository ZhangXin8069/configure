...
name: brainstorm
description: |
  需求澄清与设计探索技能：把模糊想法/粗略需求打磨成可执行的设计与规格——按
  spike（可行性探针）/bounded（小改动）/architectural（架构级）三路径分类，
  先理解上下文、一次一问澄清、提出方案权衡、分节展示设计，HARD-GATE 批准闸门：
  实现前必须先向用户说明意图并获批准，简单任务只简化设计产物、不简化批准。
  当用户要求"brainstorm"、"头脑风暴"、"先理清需求"、"设计一下"、"方案探讨"、
  "这个想法怎么做"、"需求不清楚"、"先别写代码，先想清楚"、"给几个方案选"、
  "可行性评估" 时使用此技能；需求模糊的生成类任务（"做个xxx"但细节不明）也适用。
  吸收 superpowers brainstorming 方法论，标注"spike/bounded/architectural"并先展示
  路径分类；与 plan 配合（architectural 路径产出设计后转 plan 写实现计划），
  单步实现任务不适用。
metadata:
  openclaw:
    emoji: 💡
...
2. **批准闸门是硬门（HARD-GATE）**：实现前必须先向用户说明意图并获批准；
   ...
   未检视假设导致返工最多的地方。
3. **一次一问**：每次只问一个问题，多问题拆开发问；优先选择题；聚焦
   目的/约束/成功标准。
...
```bash
ls -1t .brainstorm.*.log 2>/dev/null | head -3               # 按时间倒序(优先较新, 最多3份)
tail -80 "$(ls -1t .brainstorm.*.log 2>/dev/null | head -1)" # 预读最新一份的尾部（结论区）
```

- 目的：快速掌握已设计的项目、路径判定习惯与用户偏好（如规格存放位置），新请求直接沿用
...
2. bounded：问真正要紧的问题（目的/约束/成功标准），一次一个；
3. architectural：先问目的/约束/成功标准，再给 2-3 个方案（权衡 + 推荐）。
