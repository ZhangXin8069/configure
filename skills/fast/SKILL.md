...
本 skill 的 `fast` 是用户自定义的“多快好省”模式：通过压缩重复工作、并行独立支线和共享迭代预算，降低 Token 与资源消耗。它不等同于 Codex 运行器的原生 `fast` 模式；执行本 skill 时不得启用 Codex 原生 `fast`。后者虽然可能加速对话，但会急剧增大 Token 消耗，与本 skill 的目标相冲突。
