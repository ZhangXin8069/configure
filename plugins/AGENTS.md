...
- `install-recommended.sh` 只在用户显式执行时调用 Codex marketplace；支持 profile、单插件、Git ref、`--status/--audit` 和 dry-run，不复制第三方源码、不安装 npm 依赖、不自动信任 hooks。状态模式只读查询并禁止 `add` 命令；`--ref-policy commit` 只接受 40/64 位提交哈希。
