#!/usr/bin/env bash
# cp-small — cp 的包装：自动跳过大小大于 1MB 的文件，
...
usage() {
    ...
    echo "files larger than ${LIMIT} bytes are skipped;" >&2
    ...
    if [ -d "$src" ]; then
        ...
        while IFS= read -r -d '' entry; do
            ...
            if [ "$size" -gt "$LIMIT" ]; then
                ...
                cp "${opts[@]}" "$f" "$tgt/$rel" || {
                    echo "${_NAME}: cp failed: $f" >&2
                    return 1
                }
            fi
        done < <(find "$src" \( -type f -o -type l \) -printf '%s\t%p\0' 2>/dev/null)
