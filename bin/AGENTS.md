...
| 启动器 | `ccodex.sh`（Codex build agent：prompt 取自同目录 `ccodex-prompt.txt`；默认 `-m`=`gpt-5.6-luna`/`max`，默认权限为 Full Access（`danger-full-access`），`-o`=`max`、`-f`=`gpt-5.6-sol`/`low` 为显式快捷选项，其余旗标选择 Codex 模型；支持 `--model`、`--reasoning-effort` 与 `-file/--file`、`-time/--time` 驱动模式）、`ccodex.bat`（Windows 对应版，同一 prompt 模板，默认 `-m`）、`ccodex-snsc.sh`（HPC/snsc 入口：复用 `ccodex.sh`，默认 `-m`=`gpt-5.6-luna`/`max`，可用 `CODEX_BIN` 指定手动部署的 Codex 二进制；经 `cos` 软链接调用） |
...
- co 启动器自动注入工作目录 `AGENTS.md` 时只保留关键摘要（标题、入口/约束、命令/验证等）；完整文件仅在当前任务确有需要时按标题定向读取，以控制 prompt 上下文开销
...
- `ccodex-snsc.sh` 是 co 系列的 HPC/snsc 薄入口，默认使用 `-m`（`gpt-5.6-luna`/`max`）；不硬编码二进制路径，优先读取 `CODEX_BIN`，否则使用 PATH 中的 `codex`。Codex 没有 OpenCode `export` 等价接口，驱动模式的文件指令和「继续」消息由包装器写入 `.agent.<TS>.list`，TUI 中直接输入仍依赖 prompt 约定记录
