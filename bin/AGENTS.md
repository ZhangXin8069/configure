...
- 模型旗标与覆盖：`-h/-o/-p/-f/-q/-k/-g/-m` 选模型（默认旗标 op=`-f`、co=`-q`、cl=`-m`）；`{OPENCODE|CODEX|CLAUDE}_BIN` 指定二进制（未指定回退 PATH）、`{OPENCODE|CODEX|CLAUDE}_MODEL`/`--model MODEL` 直接覆盖模型、`{OPENCODE|CODEX|CLAUDE}_DEFAULT_MODEL_FLAG` 覆盖默认旗标、`{OPENCODE|CODEX|CLAUDE}_MODEL_M/O/P/Q/K/G/F/H` 逐旗标覆盖；op 另支持 `--variant LEVEL`（默认 `max`，`-m`=xhigh、`-h`=high，经 `OPENCODE_CONFIG_CONTENT` 注入）；co 支持 `--reasoning-effort/--sandbox/--ask-for-approval`（默认 `danger-full-access`/`never`）；各旗标模型与强度、co 的 provider/TUI 配置、op 的 agent 名或 cl 的权限模式等默认值见下条配置来源
- **agent 配置来源**：同目录 `agent-config.json`（通用：模型提供途径——`deepSeek-pay`/`opencode-go`/`custom-gpt` 的端点与 key 环境变量名、各 agent 机制参数如上下文窗口/TUI 配置/权限/沙箱/审批）+ `agent-custom.json`（个性化：各 agent 使用的供应商、模型与模型强度）深度合并后生效；`agent-custom.json` 缺失或为空时回退 `agent-custom.json.refer`（参考模板，可复制后修改；`agent-custom.json` 不入库）。途径 key 依次读 `DEEPSEEK_PAY_API_KEY`/`OPENCODE_GO_API_KEY`/`CUSTOM_GPT_API_KEY`（原 `lqcd`/`LQCD_API_KEY` 已更名 `custom-gpt`/`CUSTOM_GPT_API_KEY`；op 仅注入 key 已设置的途径，custom-gpt 另经 `custom-gpt` provider 注册端点与模型）。Unix 端用 python3 解析（可用 `AGENT_CONFIG_PYTHON` 指定解释器），Windows 端用 `ConvertFrom-Json`
