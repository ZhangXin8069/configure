...
| 启动器 | `cclaude.sh`（Claude Code）、`oopencode.sh`（opencode build agent，含固定中文 prompt；每次会话生成 `.agent.<TS>.log` 运行日志与 `.agent.<TS>.list` 用户输入清单）、`oopencode.bat`（Windows 对应版，prompt 取自同目录 `oopencode-prompt.txt` 模板）、`ddocker.sh`、`ccloudmusic.sh`、`zipython.sh`、`zjulab.sh`、`vscode_unset.sh` |
...
- `oopencode-prompt.txt` 为 `oopencode.sh` 的 PROMPT 模板（保留 `${HOME}`/`${_PWD}`/`${LIST_FILE}` 占位符，运行时展开），`oopencode.bat` 经 PowerShell 读取并替换占位符
