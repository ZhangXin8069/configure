...
- 模型旗标与覆盖：`-h/-o/-p/-f/-q/-k/-g/-m` 选模型（默认 op=`-f` DeepSeek V4 Flash 2x、co=`-q` gpt-5.5/xhigh、cl=`-m` claude-sonnet-4-5，cl 默认 slug 可用 `CLAUDE_MODEL_*` 覆盖；co 默认旗标可由 `CODEX_DEFAULT_MODEL_FLAG` 覆盖）；`{OPENCODE|CODEX|CLAUDE}_BIN` 指定二进制（未指定回退 PATH）、`{OPENCODE|CODEX|CLAUDE}_MODEL`/`--model MODEL` 直接覆盖模型；op 另支持 `--variant LEVEL`（默认 max，`-m`=xhigh、`-h`=high，经 `OPENCODE_CONFIG_CONTENT` 注入）、co 另支持 `--reasoning-effort/--sandbox/--ask-for-approval`（默认 `danger-full-access`/`never`，经 `--config` 注入）、cl 固定 `--permission-mode auto`
