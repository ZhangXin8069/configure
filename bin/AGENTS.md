...
| 启动器 | `cclaude.sh`（Claude Code）、`oopencode.sh`（opencode build agent，prompt 取自同目录 `oopencode-prompt.txt` 单一来源；每次会话生成 `.agent.<TS>.log` 运行日志与 `.agent.<TS>.list` 用户输入清单；`-h/-o/-p/-f/-q/-k/-g/-m` 选模型，默认 `-f`=DeepSeek V4 Flash (2x usage)；`-o`=Build auto · Ox Alpha Free (Unlimited) OpenCode Go·max、`-m`=Build auto·Muse Spark 1.2 Contributor OpenCode Go·xhigh、`-h`=Hy3 high 仍可显式指定；可用 `OPENCODE_BIN` 指定二进制（未指定回退 PATH 中的 opencode）、`OPENCODE_MODEL`/`--model MODEL` 直接覆盖模型、`--variant LEVEL` 覆盖 variant，各旗标默认可经 `OPENCODE_MODEL_*` 覆盖；支持 `-file/-–file PATH` 与 `-time/-–time DUR` 驱动模式，见下方「op 系列驱动模式」）、`oopencode.bat`（Windows 对应版，同一 prompt 模板，默认 `-f`=DeepSeek V4 Flash (2x usage)，同样支持驱动模式与 `--model`/`--variant` 覆盖）、`oopencode-snsc.sh`（HPC/snsc 薄入口：复用 `oopencode.sh`，默认 `-f`=DeepSeek V4 Flash，`OPENCODE_BIN` 可指定手动部署的 opencode 二进制（默认沿用 vscode-server 内 debug 二进制路径，未设置回退 PATH）；经 `ops` 软链接调用）、`ddocker.sh`、`ccloudmusic.sh`、`zipython.sh`、`zjulab.sh`、`vscode_unset.sh` |
| 启动器 | `ccodex.sh`（Codex build agent：prompt 取自同目录 `ccodex-prompt.txt`；默认 `-m`=`gpt-5.6-luna`/`max`，默认权限为 Full Access（`danger-full-access`），`-o`=`max`、`-f`=`gpt-5.6-sol`/`low` 为显式快捷选项，其余旗标选择 Codex 模型；支持 `--model`、`--reasoning-effort` 与 `-time/--time` 驱动模式）、`ccodex.bat`（Windows 对应版，同一 prompt 模板，默认 `-m`）、`ccodex-snsc.sh`（HPC/snsc 入口：复用 `ccodex.sh`，默认 `-m`=`gpt-5.6-luna`/`max`，可用 `CODEX_BIN` 指定手动部署的 Codex 二进制；经 `cos` 软链接调用） |
