...
| 启动器 | `agent.sh`（统一 agent 启动器，cpupower.sh 模式按 `$_NAME` 分发 cl/cls/op/co/ops/cos；直接调用打印 usage）、`agent.bat`（Windows 对应版，按 `%~nx0` 分发 `cl.bat`/`op.bat`/`co.bat`）、`ddocker.sh`、`ccloudmusic.sh`、`zipython.sh`、`zjulab.sh`、`vscode_unset.sh` |
...
- 模型旗标与覆盖：`-h/-o/-p/-f/-q/-k/-g/-m` 选模型（默认 op=`-f` DeepSeek V4 Flash 2x、co=`-q` gpt-5.5/xhigh、cl=`-m` claude-sonnet-4-5，cl 默认 slug 可用 `CLAUDE_MODEL_*` 覆盖；co 默认旗标可由 `CODEX_DEFAULT_MODEL_FLAG` 覆盖）；`{OPENCODE|CODEX|CLAUDE}_BIN` 指定二进制（未指定回退 PATH）、`{OPENCODE|CODEX|CLAUDE}_MODEL`/`--model MODEL` 直接覆盖模型；op 另支持 `--variant LEVEL`（默认 max，`-m`=xhigh、`-h`=high，经 `OPENCODE_CONFIG_CONTENT` 注入）、co 另默认注入 `lqcd` provider 配置（`model_provider=lqcd`、`forced_login_method=api`、`service_tier=fast`、`personality=pragmatic`、`approvals_reviewer=auto_review`、`model_context_window=1000000`、`model_auto_compact_token_limit=900000`，key 读 `LQCD_API_KEY`）；co 仍支持 `--reasoning-effort/--sandbox/--ask-for-approval`（默认 `danger-full-access`/`never`，经 `--config` 注入）、cl 固定 `--permission-mode auto`
