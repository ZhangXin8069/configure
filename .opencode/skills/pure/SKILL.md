...
- **防溢出**：代码片段每段 ≤40 行、长表用 longtable/X 列、公式用 align 分行——
  全部措施与模板第 4 节一致；
...
✓ 剖析完成
   ...
   产物:   docs/pure_<slug>_<YYYYMMDD>.pdf (N 页, X KB, Overfull=0)
...
- [ ] PDF 编译成功；`grep -c Overfull` 为 0；`pdftotext` 抽检标题/结论/清单出现
...
- 记录内容（**会话尾**）：
  5. PDF 验证结果（大小、页数、抽检文本、Overfull=0）；三态计数；结束时间与总耗时；
     遗留项与下一步建议。
