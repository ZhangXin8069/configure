...
仅参考不部署：`_docker/`（Docker 开发环境指南）、`_snsc/`（NSC 集群共享配置）、`_clash/`（Clash 代理环境安装包与 `setup.sh` 检查入口）、`_claude_code/`（指向 claude_code-v20260706 的符号链接兼容目录，含安装 `install.sh`〔默认联网下载最新版，指定版本离线包优先〕与配置 `config.sh`）、`_codex/`（Codex CLI 安装 `install.sh`/`install.bat` 与配置 `config.sh`）、`_deepseek-harness/`（DeepSeek Harness（dsh）安装 `install.sh`/`install.bat` 与配置 `config.sh`）、`_opencode/`（opencode CLI 安装与配置）、`_mac/`（macOS 更新开关脚本）、`_gitignore`（符号链接→仓库根 `.gitignore`，通用忽略模板：16 编号分节，含 C++/Java/PHP 等语言分节与「全局规则→树级放行→运行时例外」三层护栏）。
