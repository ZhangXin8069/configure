...
| 目录 | 部署到 | 用途 |
|---|---|---|
| `_bashrc` | `~/.bashrc` | Bash 配置 |
| `_zshrc` | `~/.zshrc` | Zsh 配置（oh-my-zsh，source `~/configure/env.sh`） |
| `_vimrc` | `~/.vimrc` | Vim 配置 |
| `_vim/` | `~/.vim/` | 完整 vim 运行时（plugged 插件、colors、UltiSnips、spell） |
| `_oh-my-zsh/` | `~/.oh-my-zsh/` | 完整 oh-my-zsh 安装（含 custom 插件/主题） |

仅参考不部署：`_docker/`（Docker 开发环境指南）、`_snsc/`（NSC 集群共享配置）、`_claude_code/`（指向 claude_code-v20260706 的符号链接兼容目录）、`_mac/`（macOS 更新开关脚本）。
